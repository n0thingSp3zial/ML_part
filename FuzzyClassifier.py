...
class FuzzyClassifier:
    def __init__(self, m = 2, max_iterations = 1000, eps = 1e-8, k = 0.8):
        self.u_l = None
        self.nu = None
    ...
    def train(self, train_set):
        ...
        while (True):
            ...
            if (np.linalg.norm(np.array(self.u_l) - np.array(u_l_1)) > self.eps) and (current_iteration < self.max_iterations):
                self.u_l = u_l_1
                continue
            else:
                break
    ...
    def classify(self, record):
        ...
        elem2_numer = sum([ (self.u_l[i] ** self.m) * (DTWGaussKernel(record, self.train_set[i], self.max_distance)) \
                            for i in range(len(self.train_set)) ])
        elem2_denom = self.nu * ( sum([u ** self.m for u in self.u_l]) )
        elem2 = elem2_numer / elem2_denom

        elem3 = DTWGaussKernel(record, record, self.max_distance) / self.nu
